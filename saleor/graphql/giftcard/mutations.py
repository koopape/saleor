...
class GiftCardUpdateInput(graphene.InputObjectType):
    balance = graphene.Field(PriceInput, description="Balance of the gift card.")
...
class GiftCardCreate(ModelMutation):
    ...
    def clean_input(cls, info, instance, data):
        ...
        cleaned_input["created_by"] = user if user_is_valid(user) else None
    ...
    def clean_balance(cleaned_input, currency):
        ...
        if balance:
            ...
            try:
                validate_price_precision(balance, currency)
    ...
    def post_save_action(cls, info, instance, cleaned_input):
        ...
        if "currency" in cleaned_input:
            balance_data = {
                "currency": cleaned_input["currency"],
                "initial_balance": cleaned_input["initial_balance"],
                "current_balance": cleaned_input["current_balance"],
            }
        events.gift_card_issued_event(
            user=info.context.user, app=info.context.app, balance_data=balance_data
        )
